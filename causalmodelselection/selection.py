...
class CausalSelection(BaseEstimator):
    ...
    def __init__(
        ...
        if cv is None:
            self.cv = StratifiedKFold(n_splits=5, random_state=self.random_state)
        elif cv == 1:
            self.cv = dummy1Fold()
        else:
            self.cv = cv
    ...
    def fit(self, X, y, candidate_g_estimators: List[BaseEstimator]):
        ...
        for g_estimator in candidate_g_estimators:
            ...
            g_estimator_.fit(X, y)
            self.fitted_candidates_.append(g_estimator_)

            g_estimator_r_risk = self._evaluate_candidate(X_test, y_test, g_estimator_)
    ...
    def _fit_nuisances(self, X, y):
        ...
        self.y_nuisance_estimators_cv_ = cross_validate(
            clone(self.m_model_rs_results_.best_estimator_),
            X_cov,
            y,
            cv=splitter_m,
        ...
        self.m_nuisance_estimators_ = self.y_nuisance_estimators_cv_["estimator"]
        return self
...
def cross_val_predict_from_fitted(
    estimators: List[BaseEstimator], X: np.array, A=None, cv=None, method: str = "predict"
) -> np.array:
